...
async def send_handler(message: types.Message):
    ...
    try:
        # Формат: /send ID сообщение
        parts = message.text.split(maxsplit=2)
        if len(parts) < 3:
            await message.reply("❌ Формат: `/send ID СООБЩЕНИЕ`", parse_mode='HTML')
            return
...
async def handle_query(call: types.CallbackQuery):
    ...
    if data == 'menu':
        ...
        await bot.send_message(chat_id, "<b>Вы выбрали:</b>", parse_mode='HTML')
        ...
        final_message = (
            ...
            f"🔸 Боту выпало: {choices_emoji[bot_choice]}\n\n"
            f"{result_text}\n"
            "─────────────────\n"
...
async def handle_user_input(message: types.Message):
    ...
    if state == 'awaiting_promo':
        ...
        try:
            ...
            if amount > balance:
                await message.reply(f"❌ Недостаточно средств. Ваш баланс: {balance} ⭐️")
                user_states[uid] = None
                await set_user_state(uid_int, None)
                return
            ...
            async with db_pool.acquire() as conn:
                ...
                starting_tournaments = await conn.fetch(
                    '''SELECT id, name, start_message FROM tournaments 
                       ...
                       AND start_time <= $1 
                       AND start_time > $2
                       AND start_me�ssage IS NOT NULL''',
                    now, now - 120
                )
