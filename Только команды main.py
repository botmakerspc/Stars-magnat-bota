...
async def handle_user_input(message: types.Message):
    ...
    # Use the database state if memory state is empty
    ...
    if state == 'awaiting_promo':
        ...
        print("WITHDRAW STATE:", uid, state)
        try:
        ...
        try:
            if message.sticker:
                ...
                await bot.send_animation(admin_id, message.animation.file_id, caption=f"📩 <b>Ответ от @{message.from_user.username or 'нет'} (ID {uid_int}):</b>\n\n{message.caption or ''}", parse_mode='HTML')
                await message.reply("✅ Ваша гифка успешно отправлена администратору.")
            else:
                reply_text = f"📩 <b>Ответ от пользователя @{message.from_user.username or 'нет'} (ID {uid_int}):</b>\n\n{message.text}"
